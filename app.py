...
def generar_informe_pdf(locacion_id, cliente, sitio, direccion, fecha):
    ...
    for reg in registros:
        ubicacion, marca, modelo, tipo, frigorias, refrig, filtros, drenaje, condensadora, presion, obs, img_path = reg
        ...
        c.drawString(50, y, f"Tipo: {tipo} | Frigorías: {frigorias} kcal/h | Gas: {refrig}")
        y -= 18
        ...
        y -= 15
        c.drawString(65, y, f"• Condensadora: {condensadora}   |   • Presión: {presion} PSI")
        y -= 20
        ...
        obs_txt = obs if obs else "Sin observaciones. Unidad en óptimas condiciones."
        ...
        if img_path and os.path.exists(img_path):
            try:
                ...
                c.drawImage(img, 50, y - 90, width=130, height=90, preserveAspectRatio=True)
                y -= 105
            except Exception:
...
if direccion_input:
    conn = init_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, cliente, sitio FROM locaciones WHERE direccion = %s", (direccion_input,))
    ...
    if resultado:
        ...
        sitio = st.sidebar.text_input("Nombre del Sitio (Ej: Sucursal Centro)").strip()
        
        if st.sidebar.button("Registrar Ubicación"):
            if cliente and sitio:
                cursor.execute("INSERT INTO locaciones (cliente, sitio, direccion) VALUES (%s, %s, %s)", (cliente, sitio, direccion_input))
                conn.commit()
                st.sidebar.success("¡Ubicación registrada con éxito! Recargue la página.")
                st.rerun()
            else:
    ...
    if locacion_id:
        ...
        if "1. Cargar" in menu:
            ...
            with st.form("form_equipo", clear_on_submit=True):
                ubicacion_eq = st.text_input("Ubicación del equipo (Ej: Oficina 1, Recepción)")
                marca = st.text_input("Marca")
                modelo = st.text_input("Modelo")
                tipo = st.text_input("Tipo (Split, Cassette, Central, etc.)")
                frigorias = st.number_input("Frigorías (kcal/h)", min_value=0, step=500)
                refrigerante = st.selectbox("Tipo de Refrigerante", ["R22", "R410A", "R32", "Otro"])
                consumo = st.number_input("Consumo (kW)", min_value=0.0, step=0.1)
                obs_fijas = st.text_area("Notas técnicas fijas del equipo (Opcional)")
                ...
                if submitted:
                    if ubicacion_eq and marca:
                        ...
                        ''', (locacion_id, ubicacion_eq, marca, modelo, tipo, frigorias, refrigerante, consumo, obs_fijas))
                        conn.commit()
                        st.success(f"¡Equipo en {ubicacion_eq} guardado correctamente!")
                    else:
            ...
            if not equipos:
                st.warning("No hay equipos registrados en esta dirección. Cargue equipos primero desde la opción 1.")
            else:
                with st.form("form_mantenimiento"):
                    respuestas_mantenimiento = []
                    
                    for eq in equipos:
                        eq_id, ubicacion, marca, modelo, tipo, frigorias, refrig, obs_fijas = eq
                        st.markdown(f"---")
                        st.markdown(f"📍 **UBICACIÓN: {ubicacion.upper()}** | Equipo: {marca} {modelo} ({tipo})")
                        if obs_fijas:
                            st.caption(f"Nota fija: {obs_fijas}")

                        filtros = st.selectbox(f"1. Limpieza de filtros ({ubicacion})", ["Si", "No"], key=f"f_{eq_id}")
                        drenaje = st.selectbox(f"2. Desobstrucción de drenaje ({ubicacion})", ["Si", "No"], key=f"d_{eq_id}")
                        condensadora = st.selectbox(f"3. Limpieza de condensadora ({ubicacion})", ["Si", "No", "No corresponde"], key=f"c_{eq_id}")
                        ...
                        respuestas_mantenimiento.append((eq_id, filtros, drenaje, condensadora, presion, obs, img_path))

                    submitted_mant = st.form_submit_button("Finalizar y Generar Informe PDF")
                    
                    if submitted_mant:
                        for datos in respuestas_mantenimiento:
                            eq_id, filtros, drenaje, condensadora, presion, obs, img_path = datos
                            cursor.execute('''
    ...
    st.info("👈 Ingrese la dirección del inmueble en la barra lateral para comenzar.")
